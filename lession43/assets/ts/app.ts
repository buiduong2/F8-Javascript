import type { AuthReq, AuthRes, PostReq, PostRes, RefreshTokenRes, RegisterReq, Res, UserRes } from "./types/type";
...
export class Store {
    ...
    async accessTokenToUserInfo(): Promise<void> {
        ...
        try {
            ...
            console.log(res);
            if ((res.data as Res).code === 200) {
                const authInfo = res.data.data as UserRes;
    ...
    async refreshTokenToAcessToken(): Promise<void> {
        ...
        try {
            ...
            if (res.data.code === 200) {
                const { accessToken, refreshToken } = (res.data.data as RefreshTokenRes).token;
    ...
    async login(authReq: AuthReq): Promise<void> {
        const res = await this.httpClient.post('/auth/login', { data: authReq });
        if ((res.data as Res).code === 200) {
            const authInfo = res.data.data as AuthRes;
    ...
    async getAuthInfo(): Promise<UserRes> {
        ...
        if ((res.data as Res).code === 200) {
            return res.data.data as UserRes;
        }
