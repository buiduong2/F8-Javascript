...
export class AppLink extends HTMLElement {
    ...
    addHandler() {
        const route: Route = Function(`return ${this.getAttribute("v-to")} `).call(null);
    ...
    copyAttribute() {
        ...
        Array.from(this.attributes).forEach(attr => {
            if (this.ignoreAttributesName.includes(attr.name)) return;
