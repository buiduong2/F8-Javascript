export class AppAvatarImg extends HTMLElement {
    ...
    connectedCallback() {
        if (this.isFirstRender) {
            ...
            if (!src || !userName) throw Error("v-src AND v-userName must be defined");
            ...
            Array.from(this.attributes).forEach(attr => {
                if (ignoreAttributesName.includes(attr.name)) return;
    ...
    definedFallbackAvatar(username: string, imgEl: HTMLElement): HTMLElement {
