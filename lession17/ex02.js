var distance = 50
function caculateElectricityBill(usage) {
	var totalPrice = 0
	...
	while (usage > 0 && currentStage < 5) {
		...
		if (spaceBetweenStage <= usage) {
			usage -= spaceBetweenStage
			totalPrice += currentPriceStage * spaceBetweenStage
			currentStage++
		} else {
			totalPrice += currentPriceStage * usage
			usage = 0
			break
		}
	}

	var remainDistance = usage
