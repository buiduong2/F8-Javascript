import { store } from "../index.js";
import { PageAbstract } from "../pages/PageAbstract.js";
import type { Route, RouteOption } from "../types/type.js";


export class Router {

    private routes: Route[];
    private currentPage?: PageAbstract;
    private appViewEl: HTMLElement;
    public prop: any;

    private constructor(routes: Route[], rootEl: HTMLElement) {
        this.routes = routes;
        this.appViewEl = rootEl.querySelector("app-view") as HTMLElement;
    }


    private init(): void {
        this.defineCustomeElement();
        this.renderPageFromHash();
    }

    private defineCustomeElement() {
        const tagNames = new Set<string>();
        this.routes.forEach(route => {
            const tagName = ("view" + "-" + route.ctor.name).toLowerCase();
            if (tagNames.has(tagName)) return;

            tagNames.add(tagName);
            customElements.define(tagName, route.ctor);
        })
    }

    public renderPageFromHash() {
        const [, path, id] = window.location.hash.split("/");
        const route = this.routes.find(route => route.path === "/" + path);
        if (route) {
            const canGoNext = this.filterRoute(route);
            if (canGoNext) {
                this.push({ name: route.name, params: { id } })
            }
        } else {
            this.push({ name: "Blog" });
        }
    }

    public async push(opt: RouteOption): Promise<void> {
        const nextRoute = this.getRouteByName(opt.name);
        if (nextRoute.dynamic && !opt.params?.id) throw new Error("Dynamic Route need An id");

        let nextPage;
        if (nextRoute.dynamic) {
            nextPage = PageAbstract.getNewIntance(nextRoute.ctor);
        } else {
            nextPage = PageAbstract.getIntance(nextRoute.ctor);
        }

        this.scrollToTop();

        let canGoToRoute = this.filterRoute(nextRoute, opt);
        if (!canGoToRoute || this.currentPage === nextPage) return;
        if (nextRoute.dynamic && opt.params?.id) {
            this.prop = { id: opt.params.id };
        } else {
            this.prop = {};
        }

        this.decoratorHref(nextRoute, opt);
        await nextPage.beforeRender();

        if (this.currentPage) {
            this.currentPage.replaceWith(nextPage);
        } else {
            this.appViewEl.replaceWith(nextPage);
        }
        this.currentPage = nextPage;
    }

    private filterRoute(route: Route, opt?: RouteOption): boolean {

        if (route.meta?.auth === "guest" && store.isAuth) {
            this.push({ name: "Blog" });
            return false;
        }

        if (route.meta?.auth === "auth" && !store.isAuth) {
            const query: RouteOption['query'] = { redirectTo: route.name };
            if (opt?.params?.id) {
                query.id = opt.params.id
            }

            this.push({ name: "Login", query })
            return false;
        }

        return true;
    }

    private decoratorHref(route: Route, opt: RouteOption) {
        let hash = route.path;
        if (route.dynamic) {
            hash += "/" + (opt.params?.id as string);
        }

        if (opt.query) {
            hash += "?" + new URLSearchParams(Object.entries(opt.query) as string[][]).toString();
        }
        window.location.hash = hash;
    }

    private getRouteByName(name: string): Route {
        const route = this.routes.find(r => r.name === name);
        if (route) {
            return route;
        } else {
            throw new Error("Route not found");
        }
    }

    private scrollToTop() {

        const navEl = document.querySelector("the-nav-bar");
        let top = 0;
        if (navEl) {
            top = navEl.clientTop + navEl.clientHeight;
        }
        window.scrollTo({
            behavior: "smooth",
            top,
        })
    }

    private static intance: Router;

    static createRouter(routes: Route[], rootEl: HTMLElement) {
        this.intance = new Router(routes, rootEl);
        this.intance.init();
    }

    static getIntance(): Router {
        if (!this.intance) throw new Error("Must call CreateRouter First. To setup Before get intance");
        return this.intance;
    }
}